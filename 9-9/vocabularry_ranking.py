import re
from janome.tokenizer import Tokenizer
from collections import Counter
import pandas as pd

tokenizer = Tokenizer()

KEEP_POS = {'名詞', '動詞', '形容詞'}
EXCLUDE_SUBPOS = {'非自立', '形式名詞', '接尾'}
SYMBOL_PATTERN = re.compile(r'^[^\w\u3040-\u30FF\u4E00-\u9FFF\uFF66-\uFF9F]+$', re.UNICODE)


def text2words_raw(text):
    tokens = tokenizer.tokenize(text)
    return [token.surface for token in tokens]

def text2words_filtered(text):
    tokens = tokenizer.tokenize(text)
    words = []
    for token in tokens:
        pos_info = token.part_of_speech.split(',')
        pos      = pos_info[0]
        subpos   = pos_info[1]
        base     = token.base_form

        if pos not in KEEP_POS:
            continue
        if subpos in EXCLUDE_SUBPOS:
            continue
        if base == '*' or not base:
            continue

        # 記号・絵文字を除外
        if SYMBOL_PATTERN.match(base):
            continue

        words.append(base)
    return words

def build_vocabulary(counter):
    """
    Build a word-to-index dictionary sorted by frequency (most frequent = index 0).
    
    Args:
        counter: Counter object of words
    Returns:
        dict: {word: index}
    """
    vocabullary = {}
    for index, (word, _) in enumerate(counter.most_common()):
        vocabullary[word] = index
    return vocabullary

def text2bow(text, vocab):
    """
    Convert a text string into a BoW vector based on the given vocabulary.

    Args:
        text:  input sentence (str)
        vocab: word-to-index dictionary built by build_vocabulary()
    Returns:
        list: vector of length len(vocab), each value = word count in text
    """
    vector = [0] * len(vocab)

    words = text2words_filtered(text)
    for word in words:
        if word in vocab:
            idx = vocab[word]
            vector[idx] += 1

    return vector


df = pd.read_csv('./wrime-ver2.tsv', sep='\t', usecols=['Sentence'], nrows=200)

all_words_raw      = []
all_words_filtered = []

for _, row in df.iterrows():
    all_words_raw.extend(text2words_raw(row['Sentence']))
    all_words_filtered.extend(text2words_filtered(row['Sentence']))

counter_raw      = Counter(all_words_raw)
counter_filtered = Counter(all_words_filtered)

vocabullary = build_vocabulary(counter_filtered)

# Check
sample_text = df['Sentence'].iloc[0]
bow_vector  = text2bow(sample_text, vocabullary)

print(f"Text: {sample_text}")
print(f"Vector length: {len(bow_vector)}")
print(f"Non-zero elements:")
for idx, count in enumerate(bow_vector):
    if count > 0:
        word = [w for w, i in vocabullary.items() if i == idx][0]
        print(f"  [{idx}] {word}: {count}")
